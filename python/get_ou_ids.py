...
def main():
    ...
    expected_root_ou_id = sys.argv[1]
    ou_assignments = json.loads(sys.argv[2])
    role_arn = sys.argv[3] if len(sys.argv) > 3 else None
    ...
    if expected_root_ou_id != found_root_ou_id:
        raise(Exception(f"Not in the correct AWS Org. Required: {expected_root_ou_id} Found: {found_root_ou_id}"))

    ou_results = _process_ou_assignments(boto3_client, found_root_ou_id, ou_assignments)
    print(json.dumps({"result": json.dumps(ou_results)}))

def _process_ou_assignments(boto3_client, root_ou_id, ou_assignments):
    ou_results = {}
    for path, assignments in ou_assignments.items():
        if path == '/root':
            ou_results[root_ou_id] = {'path':'/root', 'assignments': assignments}
        else:
            path = path.replace("/root", "", 1)
            ous = _get_ous(boto3_client, root_ou_id, path, "/root", root_ou_id)
            for ou in ous:
